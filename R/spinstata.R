spinstata <- function(statafile, text=NULL, keep=FALSE, ...) {
    ...
    if (is.null(text)) {
        vtext <- readLines(statafile, warn=FALSE, encoding="UTF-8")
    } else {
...
#    vtext[chunkl] <- paste0(vtext[chunkl], "\n")        # ensure trailing \n
    ...
    if (is.null(text)) {
        ...
        writeLines(vtext, rfile, useBytes=T)
        if (!keep)
...
spin_child = function(input, format) {
    ...
    knitr::asis_output(knitr::knit_child(
        text = spin_lang(text = readLines(input, encoding="UTF-8"), knit = FALSE, report = FALSE, format = fmt),
        quiet = TRUE
    ))
}
